...
def split_to_grapheme_phoneme(inp_dictionary, graphchar=True, phonchar=False, logcount=False):
  ...
  """
  # TODO: instance weighting can go here, though it's rather kludgy
  graphemes, phonemes = [], []
  for line in inp_dictionary:
    split_line = line.strip().split('\t')
    graphs = list(split_line[0]) if graphchar else split_line[0].split()
    phons = list(split_line[1]) if phonchar else split_line[1].split()
    if len(split_line) > 1:
      count = 1
      if len(split_line) > 2:
